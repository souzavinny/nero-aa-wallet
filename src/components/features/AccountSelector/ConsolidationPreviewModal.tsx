            ...
            <div className="space-y-4">
              ...
              {plan.fromAccounts.length === 0 ? (
                <div className="text-center py-4 sm:py-8 text-gray-500">
                  <svg className="w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="max-h-32 sm:max-h-64 overflow-y-auto space-y-1 sm:space-y-3">
                  {plan.fromAccounts.map((account) => (
                    <div key={account.accountId} className="border border-gray-200 rounded-lg p-2 sm:p-3">
                      <div className="flex items-center justify-between mb-1 sm:mb-2">
                        <span className="font-medium text-text-primary text-xs sm:text-sm truncate flex-1">{account.accountName}</span>
                      ...
                      <div className="space-y-0.5 sm:space-y-1">
                        {/* Native Token */}
                        {ethers.BigNumber.from(account.nativeBalance).gt(ethers.utils.parseEther('0.001')) && (
                        ...
                        {account.tokenBalances.map((tokenBalance, index) => (
                          <div key={index} className="flex justify-between text-xs">
                            <span className="text-gray-600 truncate flex-1 pr-1">{tokenBalance.token.symbol}</span>
                            <span className="font-mono">
          ...
          {plan.warnings.length > 0 && (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 sm:p-4 mb-3 sm:mb-6">
              <div className="flex items-center space-x-2 mb-1 sm:mb-2">
                <svg className="w-3 h-3 sm:w-5 sm:h-5 text-orange-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                </svg>
                <span className="text-xs sm:text-sm font-medium text-orange-800">Important Notes</span>
              </div>
